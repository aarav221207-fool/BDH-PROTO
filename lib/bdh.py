...
class BDHBlock(nn.Module):
    ...
    def apply_rope(self, x, seq_len):
        # x: [B, H, T, D_head]
        d_head = x.size(-1)
        inv_freq = 1.0 / (self.config.rope_theta ** (torch.arange(0, d_head, 2, dtype=torch.float, device=x.device) / d_head))
    ...
    def forward(self, x):
        ...
        q = self.attn_proj_q(ln_out).view(B, T, H, D_head).transpose(1, 2)
